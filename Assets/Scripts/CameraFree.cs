...
public class CameraFree : MonoBehaviour {
	...
	void Update () {
		if (Input.touchCount == 2) { //  if two fingers touched
			...
			if(one_finger == true){ // if double touch is found then swipe condition is neglected
				...
				if (end_t - start_t < maxTime) { // if time gap of swipe is valid
					transform.Rotate (new Vector3(0,(end_pos.x-start_pos.x)*rotate_rate, 0));// rotation done with proper calculation in 3d perspective
				}
...
// (end_pos.x-start_pos.x)*rotate_rate